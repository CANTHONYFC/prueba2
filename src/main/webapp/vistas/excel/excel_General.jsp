...
<%
		...
		JSONObject jsonlistado = srv.reporteExcel_General(json);
...
<tr>
				<td height='9' align='center' colspan='9' <%=centrado%> ><h2>Reporte de programación de huellas del personal</h2></td>
			</tr>			
			<tr>
			...
			<tr>
				...
				<td colspan="5" align="right"><b>ACTUALIZACI&Oacute;N: </b> <%=diaAc + "/" + mesAc + "/" + cal1.get(Calendar.YEAR) + " " + hrAc + ":" + minAc + ":" + secAc + " " + perDia%></td>
			</tr>
			<tr> 
				...
				<td width='200' <%=borderColorTitulo%> align='center'>USUARIO REGISTRO</td>
				<td width='200' <%=borderColorTitulo%> align='center'>FECHA REGISTRO</td>
				<td width='200' <%=borderColorTitulo%> align='center'>USUARIO AUTORIZADO</td>
				<td width='200' <%=borderColorTitulo%> align='center'>MODO MARCACION</td>
			...
			for(int i = 0; i<data.length();i++){
				...
				String dni=obj.get("4").toString();
					if (dni=="null") {
						...
						DNI=obj.get("4").toString();
					}
					if (obj.get("5").toString()=="null") {
						APELLIDOS="SIN APELLIDOS";} else {
						APELLIDOS=obj.get("5").toString();}
					
					if (obj.get("7").toString()=="null") {
						CARGO="SIN CARGO";} else {
					CARGO=obj.get("7").toString();}
						if (obj.get("7")==null) {
						CARGO="SIN CARGO";} else {
					CARGO=obj.get("7").toString();}
...
if (obj.get("3").toString().toString()=="null") {
						FECHA="SIN FECHA";} else {
                            FECHA=obj.get("3").toString();
}

	 	
String VALIDARHUELLA=obj.get("12").toString();
						
	if(VALIDARHUELLA.equals("NULL"))
		{
			MODO="SIN CÓDIGO";
	}else{
			MODO=obj.get("12").toString();
	}

String VALIDARESTADO=obj.get("6").toString();
if(VALIDARESTADO.equals("1"))
		...
		if (obj.get("10").toString()=="null") {
						USUARIO="SIN RESPONSABLE";} else {
					USUARIO=obj.get("10").toString();
		}

if (obj.get("11").toString()=="null") {
						 usuarioAutorizado="SIN RESPONSABLE";} else {
					 usuarioAutorizado=obj.get("11").toString();
		}
			...
			<tr> 
				...
				<td  <%=estilo2%> align='center' ><%=FECHA%></td>
                                 <td  <%=estilo2%> align='left' ><%=usuarioAutorizado%></td>
